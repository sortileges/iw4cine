/*
 *	SASS' CINEMATIC MOD - "_class" file
 *
...
init()
{
	...
	setDvarIfUninitialized("mvm_bot_customweapons", "Bots uses camos/attachs [0/1]");
	setDvar("mvm_bot_customweapons", 1 );
...
getWeaponPart( name, type )
{
	...
	if (type == "attach1")
	{
		if(isDefined(parts[1]) && parts[1] != "mp") return parts[1];
		else return "none";
	}

	if (type == "attach2")
	{
		if(isDefined(parts[2]) && parts[2] != "mp") return parts[2];
		else return "none";
	}

}

// I really messed around with giveLoadout alot
// Thank God that the class logic don't matter much in a cine mod...
giveLoadout( team, class, allowCopycat )
{

	if (isDefined(self.pers["isBot"]))
		wait 0.2;
	// ^ This fixes the stowed weapon not showing after using mvm_bot_model
	// Well, atleast sometimes, it kind of wroks randomly
	// It just frees some time so the model swap can happen before giveLoadout()
	...
	else
	{
		...
		if ( ( self.isUsingCustomLoadout == true ) && ( getDvarInt("mvm_bot_customweapons") == 0 || getDvarInt("mvm_bot_customweapons") == 1 ) )
		{
			loadoutPrimary = getWeaponPart( self.customPrimary, "name" );
			loadoutPrimaryAttachment = getWeaponPart( self.customPrimary, "attach1" );
			loadoutPrimaryAttachment2 = getWeaponPart( self.customPrimary, "attach2" );
			loadoutPrimaryCamo = GetCamoProperName(self.customPrimCamo);
			loadoutSecondary = getWeaponPart( self.customSecondary, "name" );
			loadoutSecondaryAttachment = getWeaponPart( self.customSecondary, "attach1" );
			loadoutSecondaryAttachment2 = getWeaponPart( self.customSecondary, "attach2" );
			loadoutSecondaryCamo = GetCamoProperName(self.customSecondCamo);
		}
		else if ( ( self.pers["isBot"] == false && getDvarInt("mvm_bot_customweapons") == 1 && self.isUsingCustomLoadout == false ) || ( self.pers["isBot"] == true && getDvarInt("mvm_bot_customweapons") == 0 ) ) 
		{
			loadoutPrimary = table_getWeapon( level.classTableName, class_num, 0 );
			loadoutPrimaryAttachment = table_getWeaponAttachment( level.classTableName, class_num, 0 , 0);
			loadoutPrimaryAttachment2 = table_getWeaponAttachment( level.classTableName, class_num, 0, 1 );
			...
			loadoutSecondaryAttachment = table_getWeaponAttachment( level.classTableName, class_num, 1 , 0);
			loadoutSecondaryAttachment2 = table_getWeaponAttachment( level.classTableName, class_num, 1, 1 );
			loadoutSecondaryCamo = table_getWeaponCamo( level.classTableName, class_num, 0 );
		}
		else 
		{
			loadoutPrimary = table_getWeapon( level.classTableName, class_num, 0 );
			loadoutPrimaryAttachment = giveRandomAttach("optic", loadoutPrimary);
			loadoutPrimaryAttachment2 = giveRandomAttach("other", loadoutPrimary);
			loadoutPrimaryCamo = table_getWeaponCamo( level.classTableName, class_num, 0 );
			loadoutSecondary = table_getWeapon( level.classTableName, class_num, 1 );
			loadoutSecondaryAttachment = table_getWeaponAttachment( level.classTableName, class_num, 1 , 0);
			loadoutSecondaryAttachment2 = table_getWeaponAttachment( level.classTableName, class_num, 1, 1 );
	...
	if ( !(isDefined( self.pers["copyCatLoadout"] ) && self.pers["copyCatLoadout"]["inUse"] && allowCopycat) )
	{
		...
		if ( (isCustomClass && !self isItemUnlocked( loadoutPrimary )) )
		...
		if ( (isCustomClass && !self isItemUnlocked( loadoutSecondary ) ) )
	...
	randomCamoInt = RandomIntRange( 0, 9 ); 

	if ( ( self.pers["isBot"] == false && getDvarInt("mvm_bot_customweapons") == 1 ) || ( self.pers["isBot"] == true && getDvarInt("mvm_bot_customweapons") == 0 ) || self.isUsingCustomLoadout == true ) 
		self.loadoutPrimaryCamo = int(tableLookup( "mp/camoTable.csv", 1, loadoutPrimaryCamo, 0 ));
	else self.loadoutPrimaryCamo = randomCamoInt;
	self.loadoutPrimary = loadoutPrimary;

	if ( ( self.pers["isBot"] == false && getDvarInt("mvm_bot_customweapons") == 1 ) || ( self.pers["isBot"] == true && getDvarInt("mvm_bot_customweapons") == 0 ) || self.isUsingCustomLoadout == true ) 
		self.loadoutSecondaryCamo = int(tableLookup( "mp/camoTable.csv", 1, loadoutSecondaryCamo, 0 ));
	else self.loadoutSecondaryCamo = randomCamoInt;
	...
	self.isSniper = ( weaponClass( self.primaryWeapon ) == "sniper" || weaponClass( self.secondaryWeapon ) == "sniper" );
...
giveRandomAttach( type, weapon )
{
	if ( isSubStr(weapon, "cheytac") || isSubStr(weapon, "barrett") )
		return "none"; // Better to keep the snipers w/o attachments for now, until I'm sure people want it
	if(type == "optic")
	{
		randomizer = RandomIntRange( 0, 7 );
		switch(randomizer)
		{
			case 0:
				return "reflex";
			...
			case 2:
				return "eotech";
			case 3:
				return "akimbo"; // c.f. next comment
			default:
				return "none";
		}
	}
	if( type == "other" )
	{
		// HBS and UBs will break for SMGs. The "valid attachs" checks will take care of this
		// Also, silencers are more common than other attachs, let's face it
		randomizer = RandomIntRange( 0, 9 );
		switch(randomizer)
		{
			case 0:
				return "heartbeat";
